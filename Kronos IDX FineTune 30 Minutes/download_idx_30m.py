"""Download and normalize the recent IDX 30-minute universe from Yahoo Finance."""
...
DEFAULT_OUTPUT = ROOT / "data" / "idx_kronos_all_30m.parquet"
...
def download_one(symbol: str, period: str) -> pd.DataFrame:
    frame = yf.download(
        symbol, period=period, interval="30m", auto_adjust=False,
    ...
    out = pd.DataFrame({
        "ticker": symbol.removesuffix(".JK"), "date": dt,
...
def main() -> None:
    ...
    for index, ticker in enumerate(tickers, 1):
        try:
            frame = download_one(normalize_ticker(ticker), args.period)
    ...
    if not frames:
        raise RuntimeError("Yahoo Finance tidak mengembalikan data 30 menit.")
    ...
    pd.DataFrame({"missing": missing}).to_csv(args.output.with_name("missing_30m.csv"), index=False)
